# TRACE-X SENTINEL — Cyber-Forensic Webmail Sensor

> *"Your inbox's first line of investigation."*

TRACE-X Sentinel is the browser-level companion to the **TRACE-X** cybersecurity forensic investigation platform. It runs natively inside webmail clients (**Gmail** and **Outlook Web**), extracts forensic observables in real time, generates sub-second threat verdicts backed by the TRACE-X backend engine, and enables seamless one-click deep linking into full forensic case investigations.
...
Gmail / Outlook Web (Inbox)
          │
          ▼ [Injected TRACE-X Sentinel Button & Sensor]
TRACE-X Sentinel Chrome Extension (Manifest V3)
          │
          ▼ HTTPS REST API (/api/v1/extension/analyze)
TRACE-X Forensic Backend API
          │
...
Quick Verdict Popup & Side Panel (Risk Score, Severity, Explainable "Why")
          │
          ▼ [ "INVESTIGATE IN TRACE-X →" ]
Full TRACE-X Cyber-Forensic Workstation Dashboard
```
...
1. **Native Webmail Integration**: Injects subtle, non-intrusive `[ 🛡 TRACE-X ]` action buttons into Gmail and Outlook Web email headers.
...
5. **Direct Deep Linking**: Clicking **"INVESTIGATE IN TRACE-X →"** immediately opens the exact analyzed case in the main TRACE-X workstation with reconstructed Attack Graphs, Header Flight Paths, and AI Copilot.
...
| Host Permissions | Restrict network requests strictly to `https://mail.google.com/*`, `https://outlook.live.com/*`, `https://outlook.office.com/*`, and local TRACE-X API (`http://127.0.0.1:8000/*`, `http://localhost:5173/*`). |
...
- **User-Initiated Ingestion**: Emails are analyzed only when the user clicks the TRACE-X button or opens the extension popup.
...
5. Pin the **TRACE-X Sentinel** extension to your Chrome toolbar.
6. Open any email in **Gmail** (`https://mail.google.com`) or **Outlook Web** (`https://outlook.live.com`).
7. Click the **[ 🛡 TRACE-X ]** button or extension popup to see instant forensic verdicts!
...
- **Styling**: Tailwind CSS (Dark SOC Aesthetic — `#0A0A0A` base, `#00D2BE` cyan accent, `#E10600` critical, `#22C55E` success)
- **Icons**: Lucide React
- **Testing**: Vitest
