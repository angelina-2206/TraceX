    ...
    <div className="p-3.5 rounded-lg bg-white border border-slate-200 shadow-sm space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <span className="text-base font-bold text-slate-900">{Math.round(verdict.risk_score)}</span>
      ...
      <div className="space-y-1.5 pt-2 border-t border-slate-100">
        {verdict.reasons.slice(0, 3).map((reason, idx) => (
          <div key={idx} className="flex items-start gap-1.5 text-[11px]">
            ...
            <span className="leading-snug text-slate-600">{reason}</span>
          </div>
        ))}
      </div>
      ...
      <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2">
        ...
        <button
          ...
          className="px-3 py-1.5 rounded bg-teal-600 hover:bg-teal-700 text-white text-[11px] font-semibold inline-flex items-center gap-1.5 transition-all shadow-sm"
        >
