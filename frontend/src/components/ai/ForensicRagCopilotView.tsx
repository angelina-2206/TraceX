  ...
  return (
    <div className="space-y-5 animate-fade-in font-sans">
      ...
      <div className="tracex-card p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 border-l-4 border-l-teal-500">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[11px] font-mono text-slate-400">GROUNDED EVIDENCE RETRIEVAL</span>
          </div>
        ...
        <div className="flex items-center gap-3 font-mono text-xs">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded bg-slate-900 border border-slate-800">
            ...
            <span className="text-slate-400 text-[11px]">QDRANT VECTOR:</span>
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded bg-slate-900 border border-slate-800">
            ...
            <span className="text-teal-300 font-bold">TRACE-LLM v4.2</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        <span className="text-[10px] font-mono text-slate-400 font-bold uppercase shrink-0">QUICK PROMPTS:</span>
        {PROMPT_PRESETS.map((p, idx) => {
          ...
          return (
            <button
              ...
              disabled={loading}
              className="btn-secondary text-xs py-1.5 px-3 shrink-0 flex items-center gap-1.5 font-mono"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
        ...
        <div className="lg:col-span-3 tracex-card h-[600px] flex flex-col justify-between p-5 relative">
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pr-2 font-mono text-xs">
              ...
              >
                ...
                <div className="flex items-center justify-between border-b border-white/5 pb-2 mb-3">
                  <div className="flex items-center gap-2">
                    {m.sender === 'USER' ? (
                      ...
                      <span className="font-bold text-[11px] text-teal-400 flex items-center gap-1.5">
                  ...
                  <div className="flex items-center gap-3">
                    {m.confidence && m.sender === 'AI' && (
                      <span className="text-[10px] text-teal-400 font-bold bg-teal-500/10 border border-teal-500/30 px-2 py-0.5 rounded">
                ...
                {m.evidence_references && m.evidence_references.length > 0 && (
                  <div className="flex items-center gap-1.5 flex-wrap mb-3">
                    <span className="text-[9px] text-slate-400 font-bold uppercase">CITATIONS:</span>
                ...
                <div className="whitespace-pre-wrap leading-relaxed space-y-2 text-slate-200 font-sans text-xs">
                  {m.text}
                </div>
              </div>
            ))}
            ...
            {loading && (
              <div className="p-4 rounded-xl bg-slate-950 border border-teal-500/40 font-mono text-xs text-teal-400 animate-pulse flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-2.5 h-2.5 rounded-full bg-teal-400 animate-ping" />
                  <span>Retrieving grounded evidence from case vault & Qdrant vector store...</span>
                </div>
          ...
          {/* Input Controls Bar */}
          <div className="pt-4 border-t border-white/5 flex items-center gap-3 font-mono">
            <button
            ...
            <button
              ...
              className="btn-primary text-xs py-2.5 px-4 shrink-0 flex items-center gap-2 font-mono font-bold"
            >
        ...
        <div className="space-y-4 font-mono text-xs">
          ...
          <div className="tracex-card p-4 space-y-3">
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center justify-between">
              ...
              <span className="text-teal-400 font-bold">{caseId}</span>
            </div>

            {caseDetail ? (
              <div className="space-y-2 text-[11px]">
                <div className="text-slate-100 font-bold truncate" title={caseDetail.title}>{caseDetail.title}</div>
                <div className="flex items-center justify-between text-slate-400 border-b border-slate-800 pb-1">
                  <span>SENDER:</span>
                  <span className="text-slate-200 truncate max-w-[120px]">{caseDetail.email_from}</span>
                </div>
                <div className="flex items-center justify-between text-slate-400 border-b border-slate-800 pb-1">
          ...
          <div className="tracex-card p-4 space-y-3 h-[400px] flex flex-col">
            ...
            {selectedRef ? (
              <div className="flex-1 overflow-y-auto space-y-2 text-[11px]">
                <div className="p-2.5 rounded bg-slate-900 border border-teal-500/40 text-teal-300 font-bold flex items-center justify-between">
                  <span>[{selectedRef}]</span>
                  <span className="text-[9px] text-slate-400 font-normal">VERIFIED CITATION</span>
                </div>
                <div className="text-slate-300 leading-relaxed p-3 rounded bg-slate-950 border border-slate-800 font-sans text-xs">
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-4 text-slate-500 font-sans">
                ...
                <p className="text-xs">Click any citation tag like <span className="text-teal-400 font-bold font-mono">[EV-VAULT]</span> or <span className="text-teal-300 font-bold font-mono">[QDRANT-KB]</span> in chat responses to inspect proof details.</p>
              </div>
            )}
          </div>
        </div>
