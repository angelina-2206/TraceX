  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="forensic-card p-6 flex flex-col items-center justify-center text-center space-y-4">
          ...
          <div className="relative w-36 h-36 flex items-center justify-center">
            <svg className="w-full h-full transform -rotate-90">
              ...
              <circle
                ...
                className={identity.deception_score > 60 ? 'text-red-500' : identity.deception_score > 30 ? 'text-amber-500' : 'text-emerald-500'}
                strokeLinecap="round"
                fill="transparent"
              />
            </svg>
          ...
          <span className={`px-3 py-1 rounded font-mono text-xs font-bold uppercase ${
            identity.deception_score > 60 ? 'bg-red-950 text-red-400 border border-red-800' :
            identity.deception_score > 30 ? 'bg-amber-950 text-amber-400 border border-amber-800' :
        ...
        <div className="lg:col-span-2 forensic-card p-6 space-y-4 font-mono text-xs">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-3 rounded bg-slate-950 border border-slate-800">
              <div className="text-slate-400 text-[11px]">VISIBLE DISPLAY NAME</div>
              <div className="font-semibold text-slate-100 text-sm mt-1">{identity.display_name}</div>
            </div>

            <div className="p-3 rounded bg-slate-950 border border-slate-800">
              <div className="text-slate-400 text-[11px]">CLAIMED BRAND / ROLE</div>
              <div className="font-semibold text-cyan-300 text-sm mt-1">{identity.claimed_brand || 'None Detected'}</div>
            </div>

            <div className="p-3 rounded bg-slate-950 border border-slate-800">
              <div className="text-slate-400 text-[11px]">SENDER EMAIL DOMAIN</div>
              <div className="font-semibold text-slate-200 mt-1">{identity.sender_email}</div>
            </div>

            <div className="p-3 rounded bg-slate-950 border border-slate-800">
              <div className="text-slate-400 text-[11px]">REPLY-TO DESTINATION</div>
          ...
          <div className="mt-4">
            <div className="text-slate-400 text-[11px] mb-2">DETECTED DECEPTION FACTORS ({identity.deception_factors.length})</div>
            <div className="space-y-2">
              {identity.deception_factors.map((factor, idx) => (
                <div key={idx} className="p-3 rounded bg-red-950/20 border border-red-800/40 text-red-200 flex items-start space-x-2">
