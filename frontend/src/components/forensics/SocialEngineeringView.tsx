  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="space-y-4 font-mono text-xs">
        {signals.map((s, idx) => (
          <div key={idx} className="forensic-card p-5 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <span className="font-bold text-cyan-400">SIGNAL #{idx + 1}</span>
                <span className="text-slate-100 font-semibold">{s.category}</span>
              </div>
              <span className={`px-2.5 py-0.5 rounded text-[10px] font-bold uppercase ${
                s.severity === 'HIGH' ? 'bg-red-950 text-red-400 border border-red-800' : 'bg-amber-950 text-amber-400 border border-amber-800'
              }`}>
                {s.severity} SEVERITY ({s.score.toFixed(0)}/100)
              </span>
            </div>

            <div className="p-3 rounded bg-slate-950 border border-slate-800 text-amber-200">
