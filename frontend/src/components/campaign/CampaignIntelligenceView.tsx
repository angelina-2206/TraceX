      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="tracex-card p-6 space-y-4 font-mono text-xs">
          ...
          <div className="space-y-3">
            <div>
              <span className="text-slate-500 text-[11px]">DNA HASH:</span>
              <p className="text-slate-100 font-bold text-sm mt-0.5"><code className="text-teal-300 break-all">{dna.dna_hash}</code></p>
            </div>

            <div>
              <span className="text-slate-500 text-[11px]">IDENTITY PATTERN:</span>
              <p className="text-slate-200 mt-0.5">{dna.identity_fingerprint}</p>
            </div>

            <div>
              <span className="text-slate-500 text-[11px]">REDIRECT PATTERN:</span>
              <p className="text-slate-200 mt-0.5">{dna.url_structure_hash}</p>
            </div>

            <div>
              <span className="text-slate-500 text-[11px]">AUTHENTICATION PROFILE:</span>
              <p className="text-slate-200 mt-0.5">{dna.auth_behavior_code}</p>
            </div>

            <div>
              <span className="text-slate-500 text-[11px]">INFRASTRUCTURE ASN SET:</span>
              <div className="flex items-center space-x-2 mt-1">
                {dna.infrastructure_asn_set.map(asn => (
                  <span key={asn} className="px-2 py-0.5 rounded bg-slate-900 text-teal-300 border border-slate-800 font-bold">
                    {asn}
                  </span>
                ))}
              </div>
        ...
        <div className="lg:col-span-2 tracex-card p-6 space-y-4 font-mono text-xs">
          ...
          {matches.length === 0 ? (
            ...
            <div className="space-y-4">
              {matches.map((camp) => (
                <div key={camp.campaign_id} className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-sm font-bold text-slate-100">{camp.campaign_name}</div>
                      <div className="text-[11px] text-slate-400">Match Confidence: <span className="text-teal-400 font-bold">{camp.confidence.toFixed(1)}%</span></div>
                    </div>

                    <span className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase ${
                      camp.status === 'CONFIRMED' ? 'bg-teal-500/10 text-teal-300 border border-teal-500/30' :
                      camp.status === 'REJECTED' ? 'bg-slate-900 text-slate-400 border border-slate-800' :
                  ...
                  <div className="pt-3 border-t border-slate-900 flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        ...
                        className="px-2.5 py-1 rounded bg-teal-500/10 text-teal-300 border border-teal-500/30 hover:bg-teal-500/20 text-[10px] font-bold"
                      >
                        CONFIRM MATCH
                      </button>
                      <button
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 rounded bg-amber-950/60 text-amber-300 border border-amber-800 hover:bg-amber-900 text-[10px] font-bold"
                      >
                        MARK UNCERTAIN
                      </button>
                    </div>
